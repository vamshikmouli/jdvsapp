...
export default function ReceiptPage() {
  ...
  return (
    <div className="max-w-lg mx-auto">
      {/* Print isolation + A5 sizing. Each copy is its own A5 page (parent + office). */}
      <style>{`@media print {
      ...
      <div className="no-print flex items-center justify-between py-4">
        ...
        <Button kind="primary" icon="Printer" onClick={() => window.print()}>Print (parent + office, A5)</Button>
      </div>

      {r.voided && (
        <div className="mb-3 rounded-lg bg-danger-50 border border-danger-200 px-4 py-2 text-sm text-danger-700 font-semibold text-center no-print">
          ⛔ This receipt was CANCELLED{r.voidReason ? ` — ${r.voidReason}` : ''}. The payment has been reversed.
        </div>
      )}

      <div id="print-area">
        {/* Two copies: parent keeps one, the office files the other. */}
        <ReceiptCopy r={r} copy="PARENT COPY" />
        <ReceiptCopy r={r} copy="OFFICE COPY" />
      </div>
    </div>
  );
}
...
function ReceiptCopy({ r, copy }: { r: Receipt; copy: string }) {
  return (
    <div className={`receipt-copy relative bg-white border border-slate-200 rounded-xl p-6 shadow-xs mb-6 print:mb-0 print:border-0 print:shadow-none print:rounded-none ${r.voided ? 'opacity-70' : ''}`}>
      ...
      {r.voided && (
        <div className="mb-3 rounded-lg bg-danger-50 border border-danger-200 px-4 py-1.5 text-xs text-danger-700 font-semibold text-center">
          CANCELLED{r.voidReason ? ` — ${r.voidReason}` : ''}
        </div>
      )}
