...
/** @param {{dateLabel?:string,timeLabel?:string,rows?:any[],schoolName?:string,alert?:string|null}} o */
function renderDailyBoardPng({ dateLabel, timeLabel, rows, schoolName, alert = null } = {}) {
  rows = rows || [];
  ...
  const H = top + rows.length * rowH + 60 + (alert ? 44 : 0);
  ...
  // Backup / ops alert banner (drawn only when something needs attention)
  if (alert) {
    const ay = top + rows.length * rowH + 8;
    roundRect(ctx, PAD, ay, W - PAD * 2, 32, 8); ctx.fillStyle = RED + '18'; ctx.fill();
    ctx.beginPath(); ctx.arc(PAD + 18, ay + 16, 5, 0, Math.PI * 2); ctx.fillStyle = RED; ctx.fill();
    ctx.fillStyle = RED; ctx.font = `700 14px ${FONT_BOLD}`; ctx.textAlign = 'left';
    ctx.fillText(String(alert).slice(0, 90), PAD + 32, ay + 21);
  }
